...
export default function TabButton({ label, active, onClick, icon }: TabButtonProps) {
  return (
    <button
      ...
      onClick={onClick}
    >
      {icon && <span className={`transition-colors ${active?"text-white":"text-blue-600"}`}>{icon}</span>}
